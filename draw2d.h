...
*/
/* DRAWTHICKLINE IS ACTUALLY NOT USED
...
#ifndef WIN32
void glsetpencolour(COLORREF colour) {
  float red   = ((float)COLORRED(colour)  ) / 255.0f;
  float green = ((float)COLORGREEN(colour)) / 255.0f;
  float blue  = ((float)COLORBLUE(colour) ) / 255.0f;
  glColor3f(red, green, blue);
  glPointSize(1.0);
}

float glpointx(int x, int width) {
  float glpoint = ((2.0 * x) / width) - 1.0; // double for conversion
  return glpoint; // 0 to width -> -1 to 1
}

float glpointy(int y, int height) {
  return glpointx(y, height); // same as x
}

void gldrawline(int x1, int y1, int x2, int y2) { // , COLORREF c) {
  // find equivalent GL 0-centred coordinates from pixel numbers
  float glx1 = glpointx(x1, window.width);
  float gly1 = glpointy(y1, window.height);
  float glx2 = glpointx(x2, window.width);
  float gly2 = glpointy(y2, window.height);
  glBegin(GL_LINES);
  glVertex3f(glx1, gly1, 0.0f);
  glVertex3f(glx2, gly2, 0.0f);
  glEnd();
}
#endif // exclude GL from windows build

void glmoveto(HDC dc, int xnew, int ynew) {
#ifdef WIN32
...
void gllineto(HDC dc, int xnew, int ynew) {
#ifdef WIN32
        LineTo(dc, xnew, ynew);
#else
  gldrawline(window.pen.x, window.pen.y, xnew, ynew); // , window.pen.c);
  window.pen.x = xnew;
  window.pen.y = ynew;
#endif
}

void glsetpen(HDC dc, COLORREF c) {
#ifdef WIN32
  SetDCPenColor(dc, c);
#else
  glsetpencolour(window.pen.c = c);
#endif
}

void glclearpen(HDC dc) {
#ifdef WIN32
   SelectObject(dc, GetStockObject(DC_PEN));
#else
  COLORREF blackcolour = RGB(0, 0, 0);
  glsetpencolour(window.pen.c = blackcolour);
#endif
}
...
void draw(HDC dc, int xpos, int ypos, double xscale, double yscale,
 ...
 //         printf("ROW %d COL %d\n", row, col);
      ...
      if (col == 0) {
        glmoveto(dc, xnew, ynew);
        xref = xnew; yref = ynew;
      } else {
//        COLORREF c = blackcolour;
        glclearpen(dc);
        if (phase == 1) { // latitude rings
          if (ft) { glsetpen(dc, redcolour); }
          else { glsetpen(dc, blackcolour); }
        } else if (phase == 2) { // longitude rings
          if (ft) { glsetpen(dc, bluecolour); }
          else { glsetpen(dc, greycolour); }
        } else if (phase == 3) { // selected region
          if (tlvl == row) {
            glsetpen(dc, redcolour);
          } else { glsetpen(dc, orangecolour); }
        } else if (phase == 4) { // disc ring
          if (col % 2 == 0) {
            glsetpen(dc, orangecolour);
          } else { glsetpen(dc, blackcolour); }
        } else if (phase == 5) { // disc arm
          if (col % 2 == 0) {
            glsetpen(dc, bluecolour);
          } else { glsetpen(dc, redcolour); }
        }
  ...
  //      }
//        drawLine(dc, xref, yref, xnew, ynew, c); <---- for direct draw to bitmap
        gllineto(dc, xnew, ynew); // , c);
        xref = xnew; yref = ynew;
      }
    }
  }
...
//          MoveToEx(dc, thispt.x, thispt.y, NULL);
          ...
          glclearpen(dc);
//          SelectObject(dc, GetStockObject(DC_PEN));
          glsetpen(dc, thiscolour);
//          SetDCPenColor(dc, thiscolour);
          gllineto(dc, thispt.x, thispt.y);
